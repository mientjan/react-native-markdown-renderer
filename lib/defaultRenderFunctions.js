...
const defaultRenderFunctions = {
  ...
  s: (node, children, parents, styles) => {
    return (
      <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.strikethrough, styles.strikethrough]}>
        {children}
      </Text>
    );
  },
  a: (node, children, parents, styles) => {
    return (
      <Text key={AstRenderer.getUniqueID()} style={[markdownStyles.a, styles.a]} onPress={() => AstRenderer.openUrl(node.attributes.href)}>
        {children}
      </Text>
    );
  },
